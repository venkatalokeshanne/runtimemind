...
export async function generateMetadata({ params }) {
  ...
  const canonicalUrl = `https://www.runtimemind.com/series/${slug}`;
  
  // Use cover image or generate placeholder from title
  const ogImage = series.cover_image_url || `https://www.runtimemind.com/api/og?title=${encodeURIComponent(series.title)}&type=series`;
...
export default async function SeriesDetailPage({ params }) {
  ...
  const seriesJsonLd = series ? {
    ...
    url: `https://www.runtimemind.com/series/${slug}`,
    image: series.cover_image_url || 'https://www.runtimemind.com/og-default.png',
    publisher: {
      '@type': 'Organization',
      name: 'RuntimeMind',
